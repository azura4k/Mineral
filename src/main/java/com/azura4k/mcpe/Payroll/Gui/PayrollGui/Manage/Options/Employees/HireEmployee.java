        ...
        form.responseHandler( (form, responseData) -> {
            if(!form.isClosed(responseData)) {
                try {
                    ...
                    String PlayerName = response.getInput(0);
                    String Title = response.getInput(1);
                    double Max_Minutes = Double.parseDouble(response.getInput(2));
                    int Rank = Integer.parseInt(response.getInput(3));
                    double Wage = Double.parseDouble(response.getInput(4));
